    ...
    <div className="flex min-h-svh">
      <aside className="sticky top-0 hidden h-svh w-60 shrink-0 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground md:flex">
        ...
        <nav className="mt-2 flex flex-col gap-0.5 px-3 text-sm">
          ...
          {/*
            Inspect-gateway link is hidden in production: the gateway's
            admin UI is on a Fly private port (gloop-liam-gateway.internal:
            15000) which browsers can't resolve. A public server-side
            proxy at /admin/gateway is on the M5+ todo list. For local
            dev `http://localhost:15000/ui` works directly — visit it
            in your browser without a sidebar shortcut.
          */}
